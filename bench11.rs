...
fn main() {
    ...
    let start = Instant::now();
    for _ in 0..iters {
        discover_disks_buffer_reuse();
    }
    let buffer_reuse = start.elapsed();
    println!("Buffer reuse elapsed: {:?}", buffer_reuse);

    let start = Instant::now();
    for _ in 0..iters {
        discover_disks_parallel();
    }
    let parallel = start.elapsed();
    println!("Parallel elapsed: {:?}", parallel);
}
