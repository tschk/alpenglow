...
The preferred format is GlowFS because the Soliloquy target is a read-mostly internet appliance with generation verification, browser-runtime provenance, and disposable cache semantics in the filesystem contract. EROFS remains the mature immutable fallback, and SquashFS remains the fallback for hosts or boot experiments that lack EROFS tooling.

Concrete image outputs:

- `SOLILOQUY_ROOTFS_FORMAT=glowfs system/alpine/scripts/build-rootfs-image.sh build/alpine/rootfs build/alpine/images` creates `build/alpine/images/soliloquy-rootfs.glowfs`.
...
| `/var/lib/soliloquy` | bind from `/state/var/lib/soliloquy` | bind | Browser profiles, system state, plugin state, Oil state |
...
| `/state/var/lib/soliloquy/oil` | `root:root` | `0755` |
